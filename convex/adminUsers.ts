...
export const getFilteredUsers = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.subscriptionFilter) {
      usersQuery = usersQuery.filter(q => 
        q.eq(q.field("subscriptionPlan"), args.subscriptionFilter)
      );
    }
    ...
    if (args.activityFilter || args.sortBy === "activity") {
      ...
      if (args.activityFilter) {
        if (args.activityFilter === "active") {
          users = users.filter(user => userActivityMap.get(user._id) > 0);
        } else if (args.activityFilter === "inactive") {
          users = users.filter(user => !userActivityMap.get(user._id));
        }
      }
      ...
      if (args.sortBy === "activity") {
        users.sort((a, b) => {
          const activityA = userActivityMap.get(a._id) || 0;
          const activityB = userActivityMap.get(b._id) || 0;
...
export const updateUserSubscription = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.userId, { 
      subscriptionPlan: args.subscriptionTier,
      updatedAt: Date.now()
    });
